...
export default function MaintenancePanel({ faultCodes, onOrderCreated }: MaintenancePanelProps) {
  ...
  useEffect(() => {
    async function loadStations() {
      setLoading(true)
      try {
        const data = await maintenanceApi.getStations(faultCodes)
        setStations(data)
      } catch (e) {
  ...
  const handleSelectStation = async (station: ServiceStation) => {
    setSelectedStation(station)
    setLoading(true)
    try {
      const est = await maintenanceApi.getEstimate(faultCodes, station.id)
      setEstimate(est)
      setStep('estimate')
    } catch (e) {
      console.error('获取估价失败:', e)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleConfirm = async () => {
    ...
    setSubmitting(true)
    try {
      const order = await maintenanceApi.createOrder({
        vehicleId: 'v_001',
        ...
        <div className="space-y-4">
          ...
          {loading && (
            <div className="glass-panel p-8 text-center">
              <p className="text-gray-500 animate-pulse">正在加载门店信息...</p>
            </div>
          )}
          ...
          {step === 'confirm' && selectedSlot && (
            <div className="glass-panel p-6 animate-slide-up">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={handleConfirm} disabled={submitting} className="flex-1 btn-primary disabled:opacity-50">
                  {submitting ? '提交中...' : '确认预约'}
                </button>
              </div>
            </div>
