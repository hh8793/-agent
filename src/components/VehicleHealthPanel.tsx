...
import { HealthReport, HealthIssue } from '../agents/VehicleHealthAgent'
import { healthAgent } from '../agents/VehicleHealthAgent'
import { getMockOBDData, faultCodeDB, mockMaintenanceHistory } from '../data/mockData'
...
export default function VehicleHealthPanel() {
  ...
  useEffect(() => {
    ...
    const r = healthAgent.runFullDiagnosis()
    setReport(r)
  }, [])
    ...
    <div className="space-y-6">
      ...
      <div className="glass-panel p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            { icon: Thermometer, label: '冷却液温度', value: '38°C', status: 'normal' },
            { icon: Battery, label: '电池电压', value: '12.6V', status: 'normal' },
            { icon: Droplets, label: '机油寿命', value: '85%', status: 'normal' },
            { icon: Activity, label: '故障码', value: `${report.issues.filter(i => i.faultCode).length}个`, status: report.issues.filter(i => i.faultCode).length > 0 ? 'warning' : 'normal' },
          ].map((item, i) => (
                        ...
                        <div className="space-y-3 text-xs">
                          ...
                          <div>
                            ...
                            <ul className="list-disc list-inside space-y-0.5 opacity-80">
                              {fault.possibleCauses.map((c, i) => <li key={i}>{c}</li>)}
                            </ul>
                          </div>
                          <div>
                            <p className="text-gray-500 mb-1">建议措施</p>
                            <ul className="list-disc list-inside space-y-0.5 opacity-80">
                              {fault.suggestedActions.map((a, i) => <li key={i}>{a}</li>)}
                            </ul>
                          </div>
                          {fault.urgentAction && (
      ...
      <div className="glass-panel p-6">
        ...
        <div className="space-y-2">
          {mockMaintenanceHistory.map((record, i) => (
